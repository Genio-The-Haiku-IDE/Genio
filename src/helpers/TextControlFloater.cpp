...
class MomentaryTextControl: public BTextControl {
	
	typedef	BTextControl _inherited;
	
public:
	MomentaryTextControl(BRect frame, const char* name, const char* label, const char* text,
	...
	m_sentUpdate(false) 
{
	...
	m_control = new MomentaryTextControl(
		...
		message,
		B_FOLLOW_NONE);
	m_control->ResizeTo(frame.Width(), frame.Height()+1);
...
void TextControlFloater::WindowActivated(bool activated)
{
