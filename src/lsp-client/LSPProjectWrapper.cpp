...
LSPProjectWrapper::RegisterTextDocument(LSPTextDocument* textDocument)
{
	...
	if(fileType.compare("c++") != 0 &&
	   fileType.compare("make") != 0)
	   return false;
...
LSPProjectWrapper::Dispose()
{
    ...
    for(auto& m: fTextDocs)
		LogError("LSPProjectWrapper::Dispose() still textDocument registered! [%s]", m.second->GetFilenameURI().String());
...
LSPProjectWrapper::onResponse(RequestID id, value &result)
{
	...
	if (search != fTextDocs.end()) {
	    ...
	    LogError("LSPProjectWrapper::onResponse not handled! [%s][%s] for [%s]", key.c_str(), id.c_str(), key.c_str());
	}
}
...
{
	LogError("LSPProjectWrapper::onRequest not implemented! [%s] [%s]", method.c_str(), ID.dump().c_str());
}
...
void
LSPProjectWrapper::DidChange(LSPTextDocument* textDocument, std::vector<TextDocumentContentChangeEvent> &changes,
...
RequestID
LSPProjectWrapper::Completion(LSPTextDocument* textDocument, Position position, CompletionContext& context)
{
...
RequestID
LSPProjectWrapper::TypeHierarchy(LSPTextDocument* textDocument, Position position, TypeHierarchyDirection direction, int resolve)
{
